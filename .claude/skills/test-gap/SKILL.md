...
1. Establish current coverage:
   `uv run pytest tests/unit/ -q --cov=langrank --cov-report=term-missing`
   (scope with `--cov=langrank.<subpkg>` when a path is given).
2. Spawn the **`testing-expert`** agent with the coverage output and the target path. Ask it
   ...
   (blast-radius criteria + Language Ranking module examples per tier):

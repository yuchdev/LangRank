---
name: python-expert
description: Use this agent for implementing features, bug fixes, and refactorings in Language Ranking. Use for any change to src/ or tests/. Reads the relevant ADR/ticket first, runs tests before and after, never lands a regression, and writes conventional commits. Delegate review to feature-reviewer and test authoring to testing-expert.
...
You work on implementing Language Ranking features and turn agreed designs into working, tested Python code.
...
Beyond generic payloads, this codebase handles four sensitive categories. **Raw source artifacts**: `providers/common.py::payload_from_content` writes third-party snapshots verbatim into the cache dir (`~/.cache/langrank/<provider>/`), and their redistribution terms are unresolved - `docs/roadmap/0001-generic-implementation/plan.md` §21 forbids committing large copyrighted raw datasets to Git without clear permission, and §22 requires a documented source-policy review (API availability, robots policy, terms of use, request rate, redistribution rights) before unattended fetching of a source is enabled. **User filesystem paths**: `RawArtifact.local_path` and `AppConfig.db_path`/`cache_path`/`config_path` are absolute home-directory paths that get persisted into the `raw_artifacts` table, echoed by `langrank doctor`/`status`, and leaked in tracebacks - they carry the OS username. **Survey provenance fields**: `Observation.sample_size` and `Observation.population` (populated by `providers/stackoverflow_survey.py`) are aggregate methodology metadata, not PII, but dropping, defaulting, or inventing them corrupts the record's meaning - `validate()` treats a missing or non-positive value as an `ERROR`. **Future source credentials**: no provider authenticates today and `util/http.py` sends only a `User-Agent`; the moment a provider needs a token or API key, it comes from settings/env and never from a literal, a URL query string, a cached artifact's `metadata_json`, or a `fetch_runs.error` message.

...
Seed `research_digest.exit_gates` with these even when a `plan.md` names no closing task:
...
2. **The end-to-end acceptance run** from `docs/roadmap/0001-generic-implementation/plan.md`
   ...
   `langrank export` → `langrank plot`, plus `langrank doctor`. Point it at a throwaway DB via
   `LANGRANK_DB`; never at the user's real `~/.local/share/langrank/langrank.sqlite`.
3. **A migration replay** whenever `db/migrations.py` gained a tuple: rebuild from an empty
   database and confirm `Database.schema_version()` matches `SCHEMA_VERSION`.
4. **A `security-auditor` pass scoped to the task's surface**, required only when the milestone
   touched the network fetch path (`util/http.py`), artifact caching (`providers/common.py`),
   or ingestion of a new source format. For a **new source**, plan.md §22 additionally requires
   the legal/source-policy record (official API/download availability, robots policy, terms of
   use, reasonable request rate, redistribution rights for both raw and derived data) before
   unattended fetching is enabled - treat a missing record as a failed gate.
5. **`/link-check docs/roadmap/`** - `docs/roadmap/README.md` § "Linking convention" mandates
   absolute-from-repo-root links, and this loop edits status.md, task READMEs, and new specs.

**There is no coverage floor, and the loop must not invent one.** `pyproject.toml` sets
`addopts = "-q"` with no `--cov-fail-under`, and the repo-root `.coveragerc` (`fail_under = 85`)
plus `validate_coverage.py` both target `src/aegis_swr` and a `prototype/` tree that do not
exist in this repository - they are carried over from another project and gate nothing here.
Do not treat 85% as a milestone gate unless someone first rewires it to `--cov=langrank`.
...
The default above holds: the milestone's `status.md` `## Notes & decisions` section is where a
ratified divergence is recorded, and `AskUserQuestion` is the only sign-off - this repository has
no `docs/adr/` directory and names no separate design authority, so do not block on an ADR that
does not exist. Two additions specific to this project, both part of step 3:

- If the ruling changes something the **pipeline contract** documents - the `RatingProvider`
  Protocol in `providers/base.py`, the `SourceRecord`/`Observation` field set, the observation
  natural key, or the provenance chain - also amend the mirrored prose so the next reader is not
  misled: `docs/roadmap/0001-generic-implementation/plan.md` (the relevant numbered section),
  `docs/architecture.md`, `docs/data-model.md`, `docs/providers.md`, and `CLAUDE.md`
  § "Architecture" / § "Conventions worth knowing". Delegate that to `app-architect`.
- If the ruling changes how a **source** is interpreted (a methodology break, an alias remapping,
  a provenance downgrade to reconstruction), record it in `docs/source-notes/<provider>.md`
  alongside the status.md note - that file is the per-source methodology record.

Schema divergence is never retrofitted: a ratified change to persisted shape becomes a **new**
appended `(version, sql)` tuple in `db/migrations.py`, never an edit to an applied one.
...
gated on the opt-in provider-integration tests being skipped, that is `-m "not integration"` and
must be stated explicitly rather than assumed. Third, there is **no coverage percentage and no
deployment step** - `langrank` is a locally installed CLI with no staging environment, and the
`fail_under = 85` settings in `.coveragerc`/`validate_coverage.py` point at another project's
package. Report the coverage delta as information; never block a task or milestone on a number.

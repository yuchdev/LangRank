...
- The cross-cutting rules in `@docs/dev/python_coding_standard.md` - full annotations, ruff,
  ...
  Also name `CLAUDE.md` § "Conventions worth knowing" (providers are pure over their inputs -
  no network outside `fetch()`, no DB access; never fabricate or interpolate missing values;
  flag derived values with `is_derived`/`derivation_method`; never share a plot axis across
  ratings) and `CLAUDE.md` § "Architecture" for the layer boundaries and the append-only
  `db/migrations.py` rule. There is no `AGENTS.md`, no log-redaction utility, and no logging
  framework in this repo - user-facing output goes through the `rich` `Console` in `cli.py`,
  and errors are raised as `LangRankError` subclasses from `errors.py` and caught in `main()`.
  The commands an agent must run are in `CLAUDE.md` § "Commands". If the milestone's `plan.md`
  carries its own cross-cutting guidelines section, name it for the agent to read.
- The specific anchor in `docs/roadmap/0001-generic-implementation/plan.md` the subtask
  references - the section, not the whole doc. That file is this project's detailed design
  record (there is no `docs/adr/` or `docs/specs/` yet); its top-level sections are numbered
  `# N. Title` from `# 1. Next providers` through `# 27. Long-term definition of complete`, so
  cite them as e.g. "plan.md §6 (Cross-rating comparisons)" or "§19 (Large-source performance)".
  Where the subtask concerns a specific source, add `docs/source-notes/<provider>.md`; for
  layer boundaries, schema shape, or the provider contract, add `docs/architecture.md`,
  `docs/data-model.md`, or `docs/providers.md` respectively.
...
| Condition                                                | Skill                                                                                                                                |
|----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|
| Always                                                   | `/test-gap src/langrank/<subtask-module>/` - report coverage delta; if < 85 % delegate missing tests to `testing-expert`. |
| `pyproject.toml` or any `requirements*.txt` changed      | `/dep-audit`                                                                                                                         |
| Any file under an API route, auth, or middleware path changed, or anywhere handling credentials/tokens | `/secret-scan src/langrank/<changed-module>/` |
...
- **Task complete** → this is the **one** place the whole suite runs: execute
  `uv run pytest tests/unit/ -q --cov=langrank` once (summary line only), then run
